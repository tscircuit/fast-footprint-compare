...
export const getFootprintBounds = (pads: PreviewPad[]): Bounds => {
  if (!pads.length) {
    return {
      height: 1,
      maxX: 0.5,
...
const addPadding = (bounds: Bounds) => {
...
const centerFootprint = (footprint: FootprintPreview) => {
...
export const compareFootprints = (
  ...
  const leftArea = leftCount * cellArea
  const rightArea = rightCount * cellArea
  const intersectionArea = overlapCount * cellArea
  const unionArea = unionCount * cellArea

  return {
    coverageLeft: leftArea === 0 ? 0 : intersectionArea / leftArea,
    coverageRight: rightArea === 0 ? 0 : intersectionArea / rightArea,
    gridSize,
    iou: unionArea === 0 ? 0 : intersectionArea / unionArea,
    leftOnlyRatio:
      unionArea === 0 ? 0 : Math.max(leftArea - intersectionArea, 0) / unionArea,
    normalizedLeft,
    normalizedRight,
    occupancy,
    padCountMatch: normalizedLeft.pads.length === normalizedRight.pads.length,
    rightOnlyRatio:
      unionArea === 0 ? 0 : Math.max(rightArea - intersectionArea, 0) / unionArea,
  }
}
